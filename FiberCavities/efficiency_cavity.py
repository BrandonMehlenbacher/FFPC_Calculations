...
if argument.mirror_diameter:
    arg_mir_dia = list(map(int,argument.mirror_diameter.split(",")))
if argument.radius_of_curvature:
    arg_ROC = list(map(int,argument.radius_of_curvature.split(",")))
if argument.save:
    savefig = list(argument.save.split(","))
    if len(savefig) != 5:
        print("-s requires four arguments for four plots")
        exit()
...
for radius_of_curvature in ROC_list:
    for diameter_mirror in mirror_diameter_list:
        print(f"Mirror Diameter of {diameter_mirror} and ROC of {radius_of_curvature}")
        ...
        if text:
            plt.text(x=max_cavity_length/25,y=max(efficiency(mode_radius_mirror, core_radius,radius_of_curvature,wavelength))*.01,s = f"Mirror Diameter of {diameter_mirror} and ROC of {radius_of_curvature}")
        if savefig[0] == 'y':
        ...
        plt.ylabel("Finesse")
        if text:
            plt.text(x=max_cavity_length/25,y=max(finesse)*.01,s = f"Mirror Diameter of {diameter_mirror} and ROC of {radius_of_curvature}")
        if savefig[1] == 'y':
        ...
        if text:
            plt.text(x=max_cavity_length/25,y=6,s = f"Mirror Diameter of {diameter_mirror} and ROC of {radius_of_curvature}")
        if savefig[2] == 'y':
        ...
        if text:
            plt.text(x=max_cavity_length/25,y=40,s = f"Mirror Diameter of {diameter_mirror} and ROC of {radius_of_curvature}")
        if savefig[3] == 'y':
        ...
        if text:
            plt.text(x=max_cavity_length/25,y=6,s = f"Mirror Diameter of {diameter_mirror} and ROC of {radius_of_curvature}")
        if savefig[4] == 'y':
            #plt.savefig("cooperativity/"+fig_name+"_mirror_diameter_"+str(radius_of_curvature)+"_ROC_cooperativity_Curve.png")
            plt.close()
        else:
            plt.show()
